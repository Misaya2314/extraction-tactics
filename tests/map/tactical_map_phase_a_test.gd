...
func _test_grid_schema_compatibility() -> void:
	...
	var legacy_edge_grid := GridModel.new()
	_expect(legacy_edge_grid.configure_from_definition(_schema_definition(2)), "schema: legacy edge schema 2 should load")
	var current_grid := GridModel.new()
	_expect(current_grid.configure_from_definition(_schema_definition(TacticalMapDefinition.CURRENT_SCHEMA_VERSION)), "schema: current schema 3 should load")
	var future_grid := GridModel.new()
	_expect(not future_grid.configure_from_definition(_schema_definition(TacticalMapDefinition.CURRENT_SCHEMA_VERSION + 1)), "schema: future schema should be rejected")

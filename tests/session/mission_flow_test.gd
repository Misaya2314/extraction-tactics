...
func _test_partial_and_complete() -> void:
	...
	_expect(result.success and not c.session_manager.is_terminal(), "killing final target does not end game directly")
	...
	_expect(c.session_manager.is_success(), "extraction confirms mission victory")
	_expect(count[0] == 1 and c.input_locked, "complete: one result and locked input")
	c._evaluate_mission_outcome()
	_expect(not c.session_manager.confirm_extraction() and count[0] == 1, "complete: duplicate signals ignored")

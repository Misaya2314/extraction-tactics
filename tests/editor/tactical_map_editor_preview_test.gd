...
func _run() -> void:
	...
	var encounter_label := Builder.build_encounter_label(&"enc_test_alpha", Color("ff5b5b"), 1.7)
	_expect(encounter_label != null, "preview: build_encounter_label should return a valid Label3D")
	if encounter_label != null:
		_expect(encounter_label.text == "enc_test_alpha", "preview: encounter label text should match encounter_id")
		_expect(encounter_label.billboard == BaseMaterial3D.BILLBOARD_ENABLED, "preview: encounter label should have billboard enabled")
		_expect(encounter_label.no_depth_test, "preview: encounter label should have no_depth_test enabled")
		_expect(is_equal_approx(encounter_label.position.y, 1.7), "preview: encounter label height offset should be applied")
		encounter_label.free()

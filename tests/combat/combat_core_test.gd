...
func _test_combat_resolution() -> void:
	var validation := ActionResultScript.accepted(&"player_1", &"enemy_1", 1)
...
func _test_turn_flow() -> void:
	...
	_expect(not manager.end_enemy_turn() and not manager.reset_to_exploration(), "turn: victory blocks transitions")
	manager.remove_unit(&"p1")
	_expect(manager.get_phase() == TurnManagerScript.Phase.VICTORY, "turn: terminal victory cannot be overwritten")
